...
{
    [CustomPropertyDrawer(typeof(SceneSearch))]
    public class SceneSearchAttributeDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            var attrib = this.attribute as SceneSearch;
            ...
            if (GUI.Button(position, new GUIContent(attrib.label)))
            {
                ...
                SearchWindow.Open(
                    ...
                    ScriptableObject.CreateInstance<SceneSearchProvider>().Init(t, property)
                );
            }
        }

        private Type GetType(SceneSearch attribute, Type fieldType, SerializedProperty property)
        {
